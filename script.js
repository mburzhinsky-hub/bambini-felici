const STORAGE_KEY = 'mp-analytics-v5-ozon-readfix';
...
async function parseReportFile(file, reportConfig) {
  let rawTable = null;
  let rawTableError = null;

  try {
    rawTable = await parseSpreadsheetLikeFile(file, reportConfig.hints);
  } catch (error) {
    rawTableError = error;
    if (!reportConfig.directWorkbook) {
      throw error;
    }
  }
...
function findHeaderRowByAliases(aoa, aliasGroups, maxRows = 40) {
  ...
  for (let rowIndex = 0; rowIndex < limit; rowIndex += 1) {
    ...
    const hits = aliasGroups.reduce((count, aliases) => {
      const aliasList = Array.isArray(aliases) ? aliases : [aliases];
      return count + Number(aliasList.some((alias) => normalizedRow.some((cell) => cell.includes(normalizeHeader(alias)))));
    }, 0);

    const score = hits * 100 + normalizedRow.length;
  ...
  return {
    rowIndex: bestIndex,
    hits: bestHits,
    matchedAll: bestHits >= aliasGroups.length
  };
}

function aoaToObjectsByHeaderRow(aoa, headerRowIndex) {
  if (!Array.isArray(aoa) || !aoa.length || headerRowIndex < 0 || headerRowIndex >= aoa.length) {
    return { headers: [], rows: [] };
  }

  const headers = makeUniqueHeaders(aoa[headerRowIndex] || []);
  const rows = [];
...
async function readWorkbookAoaFromFile(file, preferredSheetNames = []) {
  if (typeof XLSX === 'undefined' || !file) {
    throw new Error('Библиотека XLSX не загрузилась.');
  }

  const workbook = await readWorkbookFromFile(file);
  const normalizedPreferred = preferredSheetNames.map((name) => normalizeHeader(name));
  const preferredSheet = workbook.SheetNames.find((name) => normalizedPreferred.includes(normalizeHeader(name)));
  const sheetName = preferredSheet || workbook.SheetNames[0];
  const sheet = workbook.Sheets[sheetName];
  return { workbook, sheetName, aoa: sheetToAoaRobust(sheet) };
}

function flattenMultiRowHeaders(headerRows) {
  const width = Math.max(...headerRows.map((row) => row.length || 0), 0);
  const headers = [];
  const topCarry = [];

  for (let col = 0; col < width; col += 1) {
    const parts = [];
    headerRows.forEach((sourceRow, rowIndex) => {
      const rawValue = safeString(sourceRow[col]);
      if (rowIndex === 0) {
        if (rawValue) topCarry[col] = rawValue;
        else if (col > 0 && topCarry[col - 1]) topCarry[col] = topCarry[col - 1];
      }
      const value = rowIndex === 0 ? (rawValue || topCarry[col] || '') : rawValue;
      const normalized = normalizeHeader(value);
      if (!normalized || /^(\d+|\(.+\))$/.test(normalized)) return;
      if (!parts.some((part) => normalizeHeader(part) === normalized)) {
        parts.push(value);
      }
    });
    headers[col] = parts.join(' / ').trim() || `Колонка ${col + 1}`;
  }

  return makeUniqueHeaders(headers);
}

function collectDetectedHeadersDebug(headers) {
  return headers
    .map((header) => safeString(header))
    .filter(Boolean)
    .slice(0, 20)
    .join(' | ');
}

function parseOzonAccrualsTable(rawTable) {
  ...
  if (!columns.eventDate || !columns.group || !columns.type || !columns.amount) {
    const detected = collectDetectedHeadersDebug(rawTable.headers);
    throw new Error(`Не удалось распознать Ozon «Отчёт по начислениям». Нужны столбцы с датой начисления, группой услуг, типом начисления и суммой. Найдены заголовки: ${detected || 'не определены'}.`);
  }
...
async function parseOzonAccruals(rawTable, file) {
  if (rawTable?.headers?.length) {
    try {
      return parseOzonAccrualsTable(rawTable);
    } catch (error) {
      // fallback to direct workbook parsing below
    }
  }

  const extension = getFileExtension(file?.name || '');
  if (!['xlsx', 'xls'].includes(extension)) {
    throw new Error('Не удалось распознать Ozon «Отчёт по начислениям». Для этого файла нужен Excel с колонками: ID начисления, Дата начисления, Группа услуг, Тип начисления, Сумма итого, руб.');
  }

  const { aoa } = await readWorkbookAoaFromFile(file, ['Начисления']);
  const headerSearch = findHeaderRowByAliases(aoa, [
    ['id начисления', 'ид начисления'],
    ['дата начисления'],
    ['группа услуг'],
    ['тип начисления'],
    ['сумма итого', 'сумма итого руб']
  ], 15);

  if (!headerSearch.matchedAll) {
    const candidateHeaders = headerSearch.rowIndex >= 0 ? makeUniqueHeaders(aoa[headerSearch.rowIndex] || []) : [];
    const detected = collectDetectedHeadersDebug(candidateHeaders);
    throw new Error(`Не удалось распознать Ozon «Отчёт по начислениям». Нужны столбцы с датой начисления, группой услуг, типом начисления и суммой. Найдены заголовки: ${detected || 'не определены'}.`);
  }

  const table = aoaToObjectsByHeaderRow(aoa, headerSearch.rowIndex);
  return parseOzonAccrualsTable(table);
}
...
async function parseOzonRealization(rawTable, file, context = {}) {
  ...
  const { aoa } = await readWorkbookAoaFromFile(file, ['Отчет о реализации', 'Отчёт о реализации']);
  const headerSearch = findHeaderRowByAliases(aoa, [
    ['название товара'],
    ['артикул'],
    ['sku'],
  ...
  if (!headerSearch.matchedAll) {
    const candidateHeaders = headerSearch.rowIndex >= 0 ? flattenMultiRowHeaders([aoa[headerSearch.rowIndex] || [], aoa[headerSearch.rowIndex + 1] || []]) : [];
    const detected = collectDetectedHeadersDebug(candidateHeaders);
    throw new Error(`Не удалось распознать структуру позаказного отчёта о реализации Ozon. Найдены заголовки: ${detected || 'не определены'}.`);
  }

  const headerRowIndex = headerSearch.rowIndex;
  const flattenedHeaders = flattenMultiRowHeaders([
    aoa[headerRowIndex] || [],
    aoa[headerRowIndex + 1] || [],
    aoa[headerRowIndex + 2] || []
  ]);

  const resolver = createResolver(flattenedHeaders);
  const columns = {
    serial: resolver.find(['п п', '№ п п']),
    name: resolver.find(['название товара']),
    article: resolver.find(['артикул']),
    sku: resolver.find(['sku']),
    saleAmount: resolver.find(['реализовано реализовано на сумму руб', 'реализовано на сумму руб']),
    salePartnerPayout: resolver.find(['реализовано выплаты по механикам лояльности партнеров руб', 'реализовано выплаты по механикам лояльности партнёров руб']),
    saleDiscountPoints: resolver.find(['реализовано баллы за скидки']),
    saleQty: resolver.find(['реализовано кол во']),
    salePrice: resolver.find(['реализовано цена реализации руб']),
    baseCommissionSale: resolver.find(['реализовано базовое вознаграждение ozon руб']),
    payoutSale: resolver.find(['реализовано итого к начислению руб']),
    returnAmount: resolver.find(['возвращено клиентом возвращено на сумму руб', 'возвращено на сумму руб']),
    returnPartnerPayout: resolver.find(['возвращено клиентом выплаты по механикам лояльности партнеров руб', 'возвращено клиентом выплаты по механикам лояльности партнёров руб']),
    returnDiscountPoints: resolver.find(['возвращено клиентом баллы за скидки']),
    returnQty: resolver.find(['возвращено клиентом кол во']),
    returnPrice: resolver.find(['возвращено клиентом цена реализации руб']),
    baseCommissionReturn: resolver.find(['возвращено клиентом базовое вознаграждение ozon руб']),
    payoutReturn: resolver.find(['возвращено клиентом итого возвращено руб']),
    shipmentId: resolver.find(['отправление номер']),
    shipmentDate: resolver.find(['отправление дата'])
  };

  if (!columns.name || !columns.article || !columns.sku || !columns.saleAmount || !columns.payoutSale || !columns.shipmentId) {
    const detected = collectDetectedHeadersDebug(flattenedHeaders);
    throw new Error(`Не удалось распознать структуру позаказного отчёта о реализации Ozon. Найдены заголовки: ${detected || 'не определены'}.`);
  }
  ...
  const periodText = aoa
    ...
    .find((value) => /период|реализация товаров за период/i.test(value)) || '';
  ...
  for (let index = dataStartIndex; index < aoa.length; index += 1) {
    const row = aoa[index] || [];
    const rowObj = {};
    flattenedHeaders.forEach((header, colIndex) => {
      rowObj[header] = row[colIndex] ?? '';
    });

    const rowText = normalizeHeader(row.map((cell) => safeString(cell)).join(' '));
    if (!rowText) continue;
    if (/^итого/.test(rowText) || /^всего/.test(rowText)) break;

    const serial = safeString(getCell(rowObj, columns.serial));
    const name = safeString(getCell(rowObj, columns.name));
    const article = safeString(getCell(rowObj, columns.article));
    const sku = normalizeSku(getCell(rowObj, columns.sku));
    const shipmentId = safeString(getCell(rowObj, columns.shipmentId));

    if (!serial && !name && !article && !sku && !shipmentId) continue;

    const saleAmount = parseNumber(getCell(rowObj, columns.saleAmount)) || 0;
    const salePartnerPayout = parseNumber(getCell(rowObj, columns.salePartnerPayout)) || 0;
    const saleDiscountPoints = parseNumber(getCell(rowObj, columns.saleDiscountPoints)) || 0;
    const saleQty = Math.max(0, parseNumber(getCell(rowObj, columns.saleQty)) || 0);
    const salePrice = parseNumber(getCell(rowObj, columns.salePrice)) || 0;
    const baseCommissionSale = parseNumber(getCell(rowObj, columns.baseCommissionSale)) || 0;
    const payoutSale = parseNumber(getCell(rowObj, columns.payoutSale)) || 0;

    const returnAmount = parseNumber(getCell(rowObj, columns.returnAmount)) || 0;
    const returnPartnerPayout = parseNumber(getCell(rowObj, columns.returnPartnerPayout)) || 0;
    const returnDiscountPoints = parseNumber(getCell(rowObj, columns.returnDiscountPoints)) || 0;
    const returnQty = Math.max(0, parseNumber(getCell(rowObj, columns.returnQty)) || 0);
    const returnPrice = parseNumber(getCell(rowObj, columns.returnPrice)) || 0;
    const baseCommissionReturn = parseNumber(getCell(rowObj, columns.baseCommissionReturn)) || 0;
    const payoutReturn = parseNumber(getCell(rowObj, columns.payoutReturn)) || 0;

    rows.push({
      eventDate: anchorDate,
      reportDate: anchorDate,
      reportMonth,
      shipmentDate: parseDateValue(getCell(rowObj, columns.shipmentDate)),
      shipmentId,
      sku,
      article,
      name,
      ...
      payoutReturn,
      revenue: saleAmount - returnAmount,
      realizedRevenue: saleAmount - returnAmount,
      payoutAdjustments: salePartnerPayout + saleDiscountPoints - returnPartnerPayout - returnDiscountPoints,
...
/* ===========================
   Paste-first Ozon overrides
   =========================== */

const PASTE_STORAGE_KEY = 'mp-analytics-v6-ozon-paste';
state.pastes = {
  ozon: {
    accruals: '',
    orders: '',
...
Object.assign(MARKETPLACE_CONFIG.ozon.reports.accruals, {
  where: 'Откройте Excel-лист «Начисления».',
  why: 'Даёт логистику, эквайринг, хранение, рекламу, штрафы и прочие сервисы Ozon.',
  formatsLabel: 'Вставка из Excel',
  copyFrom: 'Лист «Начисления» в файле «Отчёт по начислениям».',
  copyHow: 'Скопируйте прямоугольный диапазон начиная со строки, где есть заголовок «ID начисления». В вашем файле это строка 2.',
  pastePlaceholder: 'Вставьте сюда диапазон из листа «Начисления» (шапка + данные).',
  usedRowsNote: 'Шапка начинается со строки 2.'
});

Object.assign(MARKETPLACE_CONFIG.ozon.reports.orders, {
  where: 'Откройте orders.csv в Excel.',
  why: 'Нужен для заказов, статусов, отмен и связки заказа с товаром / отправлением.',
  formatsLabel: 'Вставка из Excel',
  copyFrom: 'Таблица заказов, начиная с заголовка «Номер заказа».',
  copyHow: 'Скопируйте всю таблицу с первой строки. Можно вставлять как из Excel, так и сырой CSV-текст.',
  pastePlaceholder: 'Вставьте сюда таблицу заказов.',
  usedRowsNote: 'Шапка начинается с первой строки.'
});

Object.assign(MARKETPLACE_CONFIG.ozon.reports.promotion, {
  where: 'Откройте лист Statistics в файле «Аналитика продвижения».',
  why: 'Даёт рекламные расходы и рекламные метрики по SKU.',
  formatsLabel: 'Вставка из Excel',
  copyFrom: 'Лист Statistics.',
  copyHow: 'Скопируйте диапазон начиная со строки, где есть заголовок «SKU». В вашем файле это строка 2.',
  pastePlaceholder: 'Вставьте сюда лист Statistics из отчёта продвижения.',
  usedRowsNote: 'Для этой версии достаточно листа Statistics; лист Union не обязателен.'
});

Object.assign(MARKETPLACE_CONFIG.ozon.reports.returns, {
  where: 'Откройте лист «Возвраты».',
  why: 'Показывает операционные возвраты, причины возвратов и дополнительные возвратные расходы.',
  formatsLabel: 'Вставка из Excel',
  copyFrom: 'Лист «Возвраты».',
  copyHow: 'Скопируйте диапазон начиная со строки, где есть заголовок «Схема». В вашем файле это строка 6.',
  pastePlaceholder: 'Вставьте сюда таблицу возвратов.',
  usedRowsNote: 'Шапка начинается со строки 6.'
});

Object.assign(MARKETPLACE_CONFIG.ozon.reports.realization, {
  where: 'Откройте лист «Отчет о реализации».',
  why: 'Главный источник по выручке по цене продавца, базовой комиссии Ozon, к начислению и финансовым возвратам по SKU.',
  formatsLabel: 'Вставка из Excel',
  copyFrom: 'Лист «Отчет о реализации».',
  copyHow: 'Скопируйте строки 13–15 вместе с данными до конца таблицы. Нужен прямоугольный диапазон примерно A:Y.',
  pastePlaceholder: 'Вставьте сюда строки 13–15 + все строки данных из позаказного отчёта реализации.',
  usedRowsNote: 'Нужна составная шапка: строки 13, 14 и 15 вместе с данными.'
});
...
function saveState() {
  ...
  localStorage.setItem(PASTE_STORAGE_KEY, JSON.stringify(payload));
}
...
function bindEvents() {
  ...
  dom.uploadCards.addEventListener('input', (event) => {
    const textarea = event.target.closest('textarea[data-report-key]');
    if (!textarea) return;
    const reportKey = textarea.dataset.reportKey;
    state.pastes.ozon[reportKey] = textarea.value;
  });

  dom.uploadCards.addEventListener('paste', (event) => {
    const textarea = event.target.closest('textarea[data-report-key]');
    if (!textarea) return;
    const reportKey = textarea.dataset.reportKey;
    window.setTimeout(() => {
      state.pastes.ozon[reportKey] = textarea.value;
      processPastedInput(reportKey);
    }, 0);
  });

  dom.uploadCards.addEventListener('click', async (event) => {
    const processBtn = event.target.closest('[data-process-report]');
    if (processBtn) {
      const reportKey = processBtn.dataset.processReport;
      await processPastedInput(reportKey);
      return;
    }

    const clipboardBtn = event.target.closest('[data-paste-clipboard]');
    if (clipboardBtn) {
      const reportKey = clipboardBtn.dataset.pasteClipboard;
      const textarea = dom.uploadCards.querySelector(`textarea[data-report-key="${reportKey}"]`);
      if (!textarea) return;
      try {
        const text = await navigator.clipboard.readText();
        textarea.value = text;
        state.pastes.ozon[reportKey] = text;
        await processPastedInput(reportKey);
      } catch (error) {
        showToast('Браузер не дал прочитать буфер обмена. Просто вставьте таблицу в поле через Ctrl+V.', 'warn');
      }
      return;
    }
    ...
    if (clearBtn) {
      const reportKey = clearBtn.dataset.clearReport;
      state.pastes.ozon[reportKey] = '';
      state.files.ozon[reportKey] = [];
      state.lastAnalysis = null;
      renderAll();
    }
  });
  ...
  dom.buildAnalysisBtn.addEventListener('click', () => {
    const validation = getValidationState();
    if (!validation.canBuild) {
      showToast('Сначала вставьте и обработайте все обязательные отчёты Ozon.', 'warn');
      return;
    }
    buildAnalysis();
  });
  ...
  dom.resetBtn.addEventListener('click', () => {
    const confirmed = window.confirm('Сбросить все вставленные таблицы и результаты? Ручные затраты при этом сохранятся.');
    if (!confirmed) return;
    resetAllData();
  });
...
function resetAllData() {
  ...
  dom.resultsProductSearch.value = '';
  renderAll();
  showToast('Вставленные таблицы и результаты сброшены.', 'success');
}
...
function renderMarketSwitch() {
  if (!dom.marketSwitch) return;
  dom.marketSwitch.querySelectorAll('.market-btn').forEach((button) => {
    button.classList.toggle('active', button.dataset.marketplace === 'ozon');
  });
}

function renderInstructionCards() {
  const config = MARKETPLACE_CONFIG.ozon;
  const html = REPORT_SEQUENCE.ozon.map((reportKey) => {
    const report = config.reports[reportKey];
    return `
      <article class="instruction-card">
        <div class="badge-row">
          <span class="badge ${report.required ? 'required' : 'optional'}">${report.required ? 'Обязательный' : 'Опциональный'}</span>
          <span class="badge">Вставка из Excel</span>
        </div>
        <h3 class="instruction-title">${escapeHtml(report.title)}</h3>
        <p class="section-note">${escapeHtml(report.why)}</p>
        <ul class="info-list">
          <li>
            ...
            <span>${escapeHtml(report.copyFrom || report.where)}</span>
          </li>
          <li>
            <span class="info-label">Как копировать</span>
            <span>${escapeHtml(report.copyHow || '')}</span>
          </li>
          <li>
            <span class="info-label">Используемые поля</span>
            <span>${escapeHtml(report.fields.join(' • '))}</span>
...
function renderUploadCards() {
  ...
  const html = REPORT_SEQUENCE.ozon.map((reportKey) => {
    const report = config.reports[reportKey];
    const item = state.files.ozon[reportKey]?.[0];
    const text = state.pastes.ozon[reportKey] || '';

    const statusHtml = item
      ? `
        <div class="paste-status">
          <div class="paste-status-card">
            <div class="paste-status-main">
              <div class="paste-status-title">${escapeHtml(report.title)}</div>
              <div class="paste-status-text">${escapeHtml(getPasteStatusText(item))}</div>
              ${item.warnings?.length ? `<div class="paste-status-warnings">${escapeHtml(item.warnings.join(' • '))}</div>` : ''}
            </div>
            <div class="file-actions">
              <span class="badge ${item.status === 'error' ? 'error' : item.status === 'warn' ? 'partial' : 'exact'}">${item.status === 'error' ? 'Ошибка' : item.status === 'warn' ? 'Частично' : 'Готово'}</span>
            </div>
          </div>
        </div>
      `
      : '<div class="small-note">Пока ничего не вставлено.</div>';

    return `
      <article class="upload-card upload-card--paste">
        <div>
          <div class="badge-row">
            <span class="badge ${report.required ? 'required' : 'optional'}">${report.required ? 'Обязательный' : 'Опциональный'}</span>
            <span class="badge">Вставка из Excel</span>
        ...
        <div class="copy-guide">
          <div class="copy-guide-row">
            <span class="copy-guide-label">Откуда копировать</span>
            <span class="copy-guide-value">${escapeHtml(report.copyFrom || report.where)}</span>
          </div>
          <div class="copy-guide-row">
            <span class="copy-guide-label">Что выделить</span>
            <span class="copy-guide-value">${escapeHtml(report.copyHow || '')}</span>
          </div>
          <div class="copy-guide-row">
            <span class="copy-guide-label">Ожидаемые поля</span>
            <span class="copy-guide-value">${escapeHtml(report.fields.join(' • '))}</span>
          </div>
        </div>

        <textarea class="paste-box" data-report-key="${reportKey}" placeholder="${escapeHtml(report.pastePlaceholder || 'Вставьте сюда таблицу из Excel')}">${escapeHtml(text)}</textarea>

        <div class="file-actions">
          <button class="btn btn-secondary" type="button" data-paste-clipboard="${reportKey}">Вставить из буфера</button>
          <button class="btn btn-primary" type="button" data-process-report="${reportKey}">Обработать</button>
          <button class="btn btn-secondary" type="button" data-clear-report="${reportKey}">Очистить блок</button>
        </div>
...
function getPasteStatusText(item) {
  if (item.error) return item.error;
  const base = `${nfNumber.format(item.rowCount || 0)} строк распознано`;
  const monthPart = state.month ? ` • за выбранный месяц: ${nfNumber.format(item.monthRows || 0)}` : '';
...
function renderValidation() {
  ...
  dom.validationList.innerHTML = REPORT_SEQUENCE.ozon.map((reportKey) => {
    const report = config.reports[reportKey];
    const item = validation.items.find((entry) => entry.reportKey === reportKey);
    const badgeClass = item.status === 'ready' ? 'exact' : item.status === 'optional' ? 'optional' : item.status === 'error' ? 'error' : 'partial';
    const badgeLabel = item.status === 'ready' ? 'Готово' : item.status === 'optional' ? 'Опционально' : item.status === 'error' ? 'Ошибка' : 'Не готово';
    return `
      <div class="validation-item">
        <div class="validation-copy">
          <strong>${escapeHtml(report.title)}</strong>
...
function getValidationState() {
  ...
  for (const reportKey of REPORT_SEQUENCE.ozon) {
    const report = MARKETPLACE_CONFIG.ozon.reports[reportKey];
    const item = state.files.ozon[reportKey]?.[0];
    ...
    if (!item) {
      items.push({
        reportKey,
        status: report.required ? 'missing' : 'optional',
        message: report.required ? 'Таблица ещё не вставлена и не обработана.' : 'Этот отчёт можно пропустить.'
      });
      if (report.required) canBuild = false;
      continue;
    }
    ...
    if (item.status === 'error') {
      items.push({
        reportKey,
        status: 'error',
        message: item.error || 'Есть ошибка в структуре вставленной таблицы.'
      });
      if (report.required) {
        canBuild = false;
        hasBlockingErrors = true;
    ...
    let message = `Распознано ${nfNumber.format(item.rowCount || 0)} строк.`;
    if (state.month) {
      message += ` За выбранный месяц: ${nfNumber.format(item.monthRows || 0)}.`;
      if ((item.monthRows || 0) === 0 && (item.rowCount || 0) > 0) {
        message += ' Проверьте, что вставили отчёт за нужный месяц.';
      }
    }
  ...
  let statusMessage = '';
  if (canBuild) {
    statusMessage = 'Все обязательные таблицы Ozon вставлены и распознаны. Анализ можно строить.';
  } else if (hasBlockingErrors) {
    statusMessage = 'Есть ошибки в распознавании. Исправьте проблемные блоки и обработайте их заново.';
  } else {
    statusMessage = 'Не хватает обязательных таблиц для построения анализа.';
  }
...
async function processPastedInput(reportKey) {
  const text = state.pastes.ozon[reportKey] || '';
  if (!text.trim()) {
    state.files.ozon[reportKey] = [];
    state.lastAnalysis = null;
  ...
  try {
    const parsed = await parsePastedOzonReport(reportKey, text);
    const rowCount = (parsed.rows || []).length + (parsed.unionRows || []).length;
    const monthRows = countMonthRowsForReport(reportKey, parsed);
    const warnings = [...(parsed.warnings || [])];
    const status = warnings.length || (rowCount > 0 && monthRows === 0 && state.month) ? 'warn' : 'ok';

    state.files.ozon[reportKey] = [{
      id: `${reportKey}::paste`,
      name: 'Вставка из Excel',
      status,
      rowCount,
      monthRows,
      warnings,
    ...
    const validation = getValidationState();
    if (validation.canBuild) buildAnalysis(true);

    showToast(`${MARKETPLACE_CONFIG.ozon.reports[reportKey].title}: таблица распознана.`, 'success');
  } catch (error) {
    state.files.ozon[reportKey] = [{
      id: `${reportKey}::paste`,
      name: 'Вставка из Excel',
      ...
      monthRows: 0,
      warnings: [],
      error: error.message || 'Не удалось распознать таблицу.'
    }];
    state.lastAnalysis = null;
    renderAll();
    showToast(error.message || 'Не удалось распознать таблицу.', 'error', 7000);
  }
}

function refreshMonthRowsForAllReports() {
  Object.keys(state.files.ozon).forEach((reportKey) => {
    const item = state.files.ozon[reportKey]?.[0];
    if (!item || item.status === 'error') return;
    item.monthRows = countMonthRowsForReport(reportKey, item.parsed);
    if (item.status !== 'error') {
      item.status = (item.warnings?.length || ((item.rowCount || 0) > 0 && (item.monthRows || 0) === 0 && state.month)) ? 'warn' : 'ok';
    }
  });
}

function countMonthRowsForReport(reportKey, parsed) {
  const month = state.month;
  if (!parsed || !month) return parsed?.rows?.length || 0;
  const rows = parsed.rows || [];
  switch (reportKey) {
    case 'orders':
      return rows.filter((row) => isDateInSelectedMonth(row.orderDate, month)).length;
    case 'returns':
      return rows.filter((row) => isDateInSelectedMonth(row.returnDate || row.eventDate, month)).length;
    case 'promotion':
      return rows.filter((row) => isDateInSelectedMonth(row.reportDate || row.eventDate, month)).length;
    case 'realization':
      return rows.filter((row) => isDateInSelectedMonth(row.reportDate || row.eventDate || row.shipmentDate, month)).length;
    default:
      return rows.filter((row) => isDateInSelectedMonth(row.eventDate || row.orderDate || row.reportDate, month)).length;
  }
}

async function parsePastedOzonReport(reportKey, text) {
  const context = { selectedMonth: state.month };
  switch (reportKey) {
    case 'accruals':
      return parseOzonAccrualsFromPasted(text);
    case 'orders':
      return parseOzonOrdersFromPasted(text);
    case 'promotion':
      return parseOzonPromotionFromPasted(text, context);
    case 'returns':
      return parseOzonReturnsFromPasted(text);
    case 'realization':
      return parseOzonRealizationFromPasted(text, context);
    default:
...
function parsePastedMatrix(text) {
  ...
  return simpleCsvParse(clean);
}

function parseOzonAccrualsFromPasted(text) {
  const aoa = parsePastedMatrix(text);
  const headerSearch = findHeaderRowByAliases(aoa, [
    ['id начисления', 'ид начисления'],
    ['дата начисления'],
    ['группа услуг'],
  ...
  if (!headerSearch.matchedAll) {
    throw new Error('Не удалось распознать «Отчёт по начислениям». Вставьте диапазон начиная со строки, где есть заголовки «ID начисления», «Дата начисления», «Группа услуг», «Тип начисления», «Сумма итого, руб.».');
  }
...
function parseOzonOrdersFromPasted(text) {
  const aoa = parsePastedMatrix(text);
  const headerSearch = findHeaderRowByAliases(aoa, [
    ['номер заказа'],
    ['номер отправления'],
  ...
  if (!headerSearch.matchedAll) {
    throw new Error('Не удалось распознать «Отчёт по заказам». Вставьте таблицу с заголовком «Номер заказа» из Excel или откройте CSV в Excel и копируйте уже оттуда.');
  }

  const table = aoaToObjectsByHeaderRow(aoa, headerSearch.rowIndex);
  return parseOzonOrders(table);
}

async function parseOzonPromotionFromPasted(text, context = {}) {
  const aoa = parsePastedMatrix(text);
  const headerSearch = findHeaderRowByAliases(aoa, [
    ['sku'],
    ['название товара'],
  ...
  if (!headerSearch.matchedAll) {
    throw new Error('Не удалось распознать «Аналитику продвижения». Для этой версии вставляйте именно лист Statistics, начиная со строки с заголовком «SKU».');
  }

  const table = aoaToObjectsByHeaderRow(aoa, headerSearch.rowIndex);
  return parseOzonPromotion(table, null, context);
}

function parseOzonReturnsFromPasted(text) {
  const aoa = parsePastedMatrix(text);
  const headerSearch = findHeaderRowByAliases(aoa, [
    ['схема'],
    ['наименование товара'],
  ...
  if (!headerSearch.matchedAll) {
    throw new Error('Не удалось распознать «Отчёт о возвратах». Вставьте диапазон начиная со строки, где есть заголовок «Схема».');
  }

  const table = aoaToObjectsByHeaderRow(aoa, headerSearch.rowIndex);
  return parseOzonReturns(table);
}

function parseOzonRealizationFromPasted(text, context = {}) {
  const aoa = parsePastedMatrix(text);
  const headerSearch = findHeaderRowByAliases(aoa, [
    ['название товара'],
    ['артикул'],
  ...
  if (!headerSearch.matchedAll) {
    throw new Error('Не удалось распознать «Позаказный отчёт о реализации». Вставьте строки 13–15 вместе с данными, чтобы приложение увидело составную шапку.');
  }
  ...
  const columns = {
    ...
    sku: resolver.find(['sku']),
    saleAmount: resolver.find(['реализовано реализовано на сумму руб', 'реализовано на сумму руб']),
    salePartnerPayout: resolver.find(['реализовано выплаты по механикам лояльности партнеров руб', 'реализовано выплаты по механикам лояльности партнёров руб', 'реализовано выплаты по механикам лояльности']),
    saleDiscountPoints: resolver.find(['реализовано баллы за скидки']),
    saleQty: resolver.find(['реализовано кол во']),
    salePrice: resolver.find(['реализовано цена реализации руб']),
    baseCommissionSale: resolver.find(['реализовано базовое вознаграждение ozon руб']),
    ...
    payoutReturn: resolver.find(['возвращено клиентом итого возвращено руб']),
    shipmentId: resolver.find(['отправление номер']),
    shipmentDate: resolver.find(['отправление дата', 'дата'])
  };

  if (!columns.name || !columns.article || !columns.sku || !columns.saleAmount || !columns.payoutSale || !columns.shipmentId) {
    throw new Error('Позаказный отчёт распознан частично. Проверьте, что вставлены строки 13–15 и все колонки A:Y.');
  }

  const warnings = [];
  const reportMonth = context.selectedMonth || '';
  const anchorDate = monthAnchorDate(reportMonth || context.selectedMonth);
  const rows = [];
  const dataStartIndex = headerRowIndex + 3;
...
function analyzeOzon() {
  ...
  const ordersPlaced = uniqueBy(monthOrderRows, (row) => row.orderId || row.shipmentId).length;
  const canceledOrders = uniqueBy(monthOrderRows.filter((row) => row.isCancel), (row) => row.orderId || row.shipmentId).length;
  const buyoutOrders = uniqueBy(realizationRows.filter((row) => (row.saleQty || 0) > 0), (row) => row.shipmentId || `${row.sku}-${row.article}-${row.name}`).length;
  ...
  const dailySeries = buildDailySeries(
    realizationRows.map((row) => ({
      date: row.shipmentDate || row.reportDate || row.eventDate,
      value: (row.saleAmount || 0) - (row.returnAmount || 0)
    })),
    month
  );
  ...
  const limitations = [
    'Импорт сделан через вставку диапазона из Excel. Это сознательно надёжнее, чем чтение исходных xlsx/xls в браузере.',
    'Позаказный отчёт о реализации задаёт товарную выручку по цене продавца, сумму к начислению и финансовые возвраты.',
    'Отчёт по начислениям используется как слой расходов Ozon поверх товарной выручки: логистика, эквайринг, продвижение, штрафы, хранение и прочие сервисы.',
    'Если рекламный файл загружен, расходы за «Оплату за клик» берутся из него, чтобы точнее разложить рекламу по SKU и не задвоить начисления.',
    'Отчёт о возвратах показывает операционные возвраты, которые могут быть шире финансовых возвратов в позаказной реализации.',
  ...
  return {
    ...
    month,
    title: 'Ozon',
    subtitle: 'Картина собрана из пяти слоёв Ozon: начисления, заказы, продвижение, возвраты и позаказная реализация. Импорт — через вставку диапазонов из Excel.',
    availability,
    limitations,
    kpis: [
      makeKpi('Выручка', revenue, 'точно', 'По цене продавца, уже с учётом финансовых возвратов', 'money'),
      makeKpi('К начислению по товарам', payoutBeforeServices, 'точно', 'После базовой комиссии Ozon, до логистики и прочих сервисов', 'money'),
      makeKpi('Заказы', ordersPlaced, 'точно', 'По номеру заказа в выгрузке заказов за выбранный месяц', 'number'),
      makeKpi('Выкуплено товаров', buyoutUnits, 'точно', 'По позаказной реализации', 'number'),
      makeKpi('Отмены', canceledOrders, 'частично', 'По текущему статусу заказа в выгрузке заказов', 'number'),
      makeKpi('Финансовые возвраты', financialReturnUnits, 'точно', 'По строкам возвратов в позаказной реализации', 'number'),
      makeKpi('Операционные возвраты', operationalReturnUnits, 'точно', 'По отдельному отчёту возвратов Ozon', 'number'),
      makeKpi('Средний чек', averageCheck, 'точно', 'Выручка / число отправлений в позаказной реализации', 'money'),
      makeKpi('Расходы маркетплейса', linkedMarketCosts + commonCosts, 'точно', 'Логистика, эквайринг, продвижение, хранение, штрафы и прочие сервисы', 'money'),
      makeKpi('Итог после ручных затрат', netResult, totalManualCosts ? 'точно' : 'частично', totalManualCosts ? 'Включая ручной блок затрат' : 'Себестоимость не заполнена или заполнена частично', 'money')
    ],
    dailySeries,
    expenses,
