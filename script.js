...
const ARTICLE_COST_SEARCH = document.getElementById('articleCostSearch');
const ARTICLE_COST_PASTE = document.getElementById('articleCostPaste');
const APPLY_ARTICLE_COST_PASTE_BTN = document.getElementById('applyArticleCostPasteBtn');
const CLEAR_ARTICLE_COSTS_BTN = document.getElementById('clearArticleCostsBtn');
...
function init() {
  ...
  ARTICLE_COST_SEARCH.addEventListener('input', renderArticleCostTable);
  APPLY_ARTICLE_COST_PASTE_BTN.addEventListener('click', applyArticleCostPaste);
  CLEAR_ARTICLE_COSTS_BTN.addEventListener('click', clearArticleCosts);
...
function loadSettings() {
  try {
    ...
    return {
      ...
      articleCosts: parsed.articleCosts || {},
...
function buildAnalysis(dataset, settings) {
  ...
  products.forEach((item) => {
    ...
    const articleKey = normalizeArticle(item.article);
    const articleCostOverride = articleKey ? state.settings.articleCosts[articleKey] : undefined;
    const hasSkuCostOverride = hasExplicitValue(skuCostSettings.costPrice);
    const hasArticleCostOverride = hasExplicitValue(articleCostOverride);
    const unitCostPrice = hasSkuCostOverride
      ? toNumber(skuCostSettings.costPrice)
      : hasArticleCostOverride
        ? toNumber(articleCostOverride)
        : toNumber(settings.defaults.costPrice);
    ...
    item.costPricePerUnit = unitCostPrice;
    item.packagingPerUnit = unitPackaging;
    item.inboundPerUnit = unitInbound;
    item.otherVarPerUnit = unitOther;
    item.costSource = hasSkuCostOverride ? 'sku' : hasArticleCostOverride ? 'article' : 'default';
  ...
  products.forEach((item) => {
    ...
    item.profitPerUnit = item.netUnits > 0 ? item.finalContribution / item.netUnits : 0;
  });
  ...
  return {
    ...
    articleRows: buildArticleRows(productList, settings),
    totals,
...
function buildArticleRows(productList, settings) {
  const grouped = new Map();

  productList.forEach((item) => {
    const rawArticle = String(item.article || '').trim();
    const articleKey = normalizeArticle(rawArticle) || `__sku__${item.sku}`;

    if (!grouped.has(articleKey)) {
      grouped.set(articleKey, {
        articleKey,
        article: rawArticle || `Без артикула · SKU ${item.sku}`,
        name: item.name || '',
        skuCount: 0,
        skus: [],
        netUnits: 0,
        ...
        directContribution: 0,
        finalContribution: 0,
        hasSkuOverride: false,
        synthetic: !rawArticle
      });
    }

    const row = grouped.get(articleKey);
    row.skuCount += 1;
    row.skus.push(item.sku);
    row.netUnits += item.netUnits || 0;
    row.netSales += item.netSales || 0;
    row.manualCosts += item.manualCosts || 0;
    row.directContribution += item.directContribution || 0;
    row.finalContribution += item.finalContribution || 0;
    row.hasSkuOverride = row.hasSkuOverride || item.costSource === 'sku';
    if (!row.name && item.name) row.name = item.name;
  });

  return Array.from(grouped.values())
    .map((row) => {
      const savedCost = row.synthetic ? undefined : state.settings.articleCosts[row.articleKey];
      return {
        ...row,
        savedCost: hasExplicitValue(savedCost) ? toNumber(savedCost) : '',
        manualCostPerUnit: row.netUnits > 0 ? row.manualCosts / row.netUnits : 0,
        profitPerUnit: row.netUnits > 0 ? row.finalContribution / row.netUnits : 0,
        fallbackCost: toNumber(settings.defaults.costPrice)
      };
    })
...
function renderKpis(analysis) {
  ...
  const totalNetUnits = analysis.products.reduce((sum, item) => sum + (item.netUnits || 0), 0);
  const contributionBase = analysis.allocationMode === 'none' ? analysis.totals.directContribution : analysis.totals.finalContribution;
  const avgProfitPerUnit = totalNetUnits > 0 ? contributionBase / totalNetUnits : 0;
  const items = [
    ...
    { label: 'Получаю в среднем с 1 игрушки', value: formatMoney(avgProfitPerUnit), foot: 'После всех расходов и выбранного правила распределения.' },
    ...
    { label: analysis.allocationMode === 'none' ? 'Вклад до общих расходов' : 'Вклад после распределения', value: formatMoney(contributionBase), foot: analysis.allocationMode === 'none' ? 'Общие расходы без SKU не распределены по товарам.' : 'С учётом выбранного правила распределения.' }
  ];
...
function renderArticleCostTable() {
  if (!state.analysis) return;

  const thead = document.querySelector('#articleCostTable thead');
  const tbody = document.querySelector('#articleCostTable tbody');
  ...
  thead.innerHTML = `
    <tr>
      ...
      <th>Ручные затраты / шт</th>
      <th>Получаю с 1 игрушки</th>
      <th>Итоговый вклад</th>
    </tr>
  `;

  const search = ARTICLE_COST_SEARCH.value.trim().toLowerCase();
  const rows = state.analysis.articleRows.filter((row) => {
    if (!search) return true;
    return [row.article, row.name, row.skus.join(' ')].some((value) => String(value || '').toLowerCase().includes(search));
  });

  tbody.innerHTML = rows.map((row) => {
    const profitClass = row.profitPerUnit >= 0 ? 'positive' : 'negative';
    const note = row.hasSkuOverride
      ? 'Есть SKU-переопределения'
      : row.synthetic
        ? 'Нет артикула в книге'
        : row.savedCost === ''
          ? 'Используется значение по умолчанию'
          : 'Сохранено для артикула';
    const skuText = row.skuCount === 1 ? row.skus[0] : `${row.skuCount} SKU`;
    return `
      <tr>
        <td class="cell-strong">${escapeHtml(row.article)}</td>
        <td>
          <div class="article-cell-stack">
            <span>${escapeHtml(shorten(row.name || '—', 52))}</span>
            <span class="article-cell-sub">${escapeHtml(note)}</span>
          </div>
        </td>
        <td>${escapeHtml(skuText)}</td>
        <td>${escapeHtml(formatInteger(row.netUnits))}</td>
        <td>
          <input
            class="inline-input"
            data-article-key="${escapeHtml(row.articleKey)}"
            type="number"
            step="0.01"
            value="${safeInputValue(row.savedCost)}"
            placeholder="${row.fallbackCost ? String(row.fallbackCost) : 'по умолчанию'}"
            ${row.synthetic ? 'disabled' : ''}
          />
        </td>
        <td>${escapeHtml(formatMoney(row.manualCostPerUnit))}</td>
        <td class="article-profit ${profitClass}">${escapeHtml(formatMoney(row.profitPerUnit))}</td>
        <td class="${row.finalContribution >= 0 ? 'cell-positive' : 'cell-negative'}">${escapeHtml(formatMoney(row.finalContribution))}</td>
      </tr>
    `;
  }).join('');

  tbody.querySelectorAll('input[data-article-key]').forEach((input) => {
    input.addEventListener('change', handleArticleCostInput);
  });
}

function handleArticleCostInput(event) {
  const { articleKey } = event.target.dataset;
  const value = event.target.value;
  if (!articleKey) return;

  if (value === '') {
    delete state.settings.articleCosts[articleKey];
  } else {
    state.settings.articleCosts[articleKey] = toNumber(value);
  }
...
function applyArticleCostPaste() {
  const text = ARTICLE_COST_PASTE.value.trim();
  if (!text) {
    alert('Вставь список из Excel: артикул и себестоимость в двух колонках.');
    return;
  }
  ...
  rows.forEach((row, index) => {
    ...
    const rawCost = row[1];
    const rawCostText = String(rawCost ?? '').trim();

    if (!article && !rawCostText) return;

    const looksLikeHeader = index === 0
      && /артикул/i.test(article)
      && /себ|стоим|cost/i.test(String(row.slice(1).join(' ')));
    if (looksLikeHeader) return;

    if (!article || !hasExplicitValue(rawCostText)) {
      skipped += 1;
      return;
    }

    state.settings.articleCosts[normalizeArticle(article)] = toNumber(rawCostText);
    applied += 1;
  });

  persistSettings();
  ARTICLE_COST_PASTE.value = '';
  if (state.analysis) rebuildAnalysis();

  alert(`Применено артикулов: ${applied}${skipped ? ` · пропущено строк: ${skipped}` : ''}`);
}

function clearArticleCosts() {
  if (!Object.keys(state.settings.articleCosts || {}).length) {
    alert('Список себестоимости по артикулам уже пуст.');
    return;
  }
  if (!confirm('Очистить все сохранённые себестоимости по артикулам?')) return;
  state.settings.articleCosts = {};
  persistSettings();
  if (state.analysis) rebuildAnalysis();
}
...
const TABLE_COLUMNS = [
  ...
  { key: 'costPricePerUnit', label: 'Себест./шт', type: 'money' },
  { key: 'manualCostPerUnit', label: 'Все ручн./шт', type: 'money' },
  { key: 'profitPerUnit', label: 'Получаю с 1 шт', type: 'money' },
  ...
  { key: 'manualInputs', label: 'SKU-переопр.', type: 'manual' }
];
...
function renderProductRow(item) {
  const cells = TABLE_COLUMNS.map((column) => {
    ...
    if (column.type === 'money') {
      ...
      if (rawValue > 0 && ['finalContribution', 'profitPerUnit'].includes(column.key)) className = 'cell-positive';
...
function exportProductCsv() {
  if (!state.analysis) return;
  const headers = [
    'SKU','Артикул','Товар','Чистые ед.','Себест./шт','Все ручн./шт','Получаю с 1 шт','Заказы','Отмены','Операционные возвраты','Выручка нетто','К начислению',
  ...
  const rows = state.analysis.products.map((item) => [
    item.sku, item.article, item.name, item.netUnits, item.costPricePerUnit, item.manualCostPerUnit, item.profitPerUnit, item.ordersCount, item.cancelledQty, item.operationalReturnsQty, item.netSales,
